...
**Fonction principale : `get_chunks()`**
...
**Objets importants :**
- `all_chunks` (list) : Liste contenant tous les segments de texte avec leurs métadonnées
- `chunk_obj` (dict) : Dictionnaire individuel contenant les clés (source_file, index et text)


2. FICHIER indexation.py : Indexation dans Upstash Vector 

**Fonction principale : `index_chunks(chunks: list) -> str`**
...
**Objets importants :**
- `Index` : Classe de gestion de la base de données vectorielle Upstash
- `vectors_data` (list) : Liste structurée contenant les vecteurs (id, données et métadonnées)
- `chunk_id` (str) : Identifiant unique généré pour chaque chunk


3. Agent RAG -> agentAI.py 

**Fonction de recherche : `recherche_portfolio(query: str) -> str`**
C'est l'outil de recherche de l'IA : le "RAG" (Retrieval-Augmented Generation). 
...
**Objets importants :**
- `results` (list) : Liste des résultats retrouvés avec métadonnées et scores
- `top_k` : Nombre de résultats les plus pertinents à récupérer (3 dans mon code)
- `metadata` : Dictionnaire contenant les informations associées au chunk
- `score` (float) : Score de pertinence entre le résultat trouvé et la demande/question de l'utilisateur


**Fonction d'orchestration : `run_agent_sync(user_message: str) -> str`**
...
**Objets importants :**
- `mon_agent` : Agent configuré avec le modèle "gpt-4-nano" 
- `Runner.run_sync()` : Fonction qui exécute l'agent de manière synchrone pour être compatible avec Streamlit
- `result` : Objet contenant la réponse de l'agent avec l'attribut `final_output`
...
**Objets importants :**
- `st.chat_message()` : Composant Streamlit qui affiche les messages du chat
- `st.chat_input()` : Barre de saisie pour récupérer les messages/questions des utilisateurs
- `st.session_state` : Mémoire de session pour conserver l'historique du chat
- `st.session_state.messages` (list) : Historique des échanges entre l'utilisateur et le chatbot
